...
* **Micro-State (Valuation) $\sigma \in \Sigma$**: The map of variables to STP tensors (e.g., $\{ n \mapsto \text{Odd} \}$).
...
The `Branch` action induces a 1-to-N transition. Let $a = \text{Branch}(C_1, C_2)$：
...
Let $\pi: A^* \to T$ be the parsing map. The system is valid iff the generated linear sequence $u \in A^*$ maps to a tree $T$ such that every edge in $T$ satisfies the local STP relation $\mathcal{R}$.
...
struct STPContext {
    ...
    stack: Vec<ContextFrame>, // \Gamma (Context Stack)
}
...
impl STPContext {
    fn transition(&mut self, action: ProofAction) {
        match action {
            ...
            QED => self.pop_and_merge(), // \oplus_logic (Logical Join)
